...
<html>
    ...
    <body>
        <%  pageContext.setAttribute("variables", request.getAttribute("employeeList")); 
            ...
            pageContext.setAttribute("transferredInListVar", request.getAttribute("transferredInList")); 
        ...
        <div id="Transferred" style="display: none;position: fixed ; width: 70%;height: 70vh;top: 50%;
            ...
            <form action="ChangeDepartment">
                <table style="text-align: center" class="empTable">
                    ...
                    <input type='hidden' name='empCatId' <% out.print("value='"+emp.getEmpJobCategory().getCatId()+"'"); %>/>
                    <input type='hidden' name='sendFrom' value="director_one_answer"/>
                    <input type="number" name="empId" required="true" />
        ...
        <input id="showOtBtn" type="button" value="Show Transferred Out List" onclick="showOtDialog()" />   

        <div id="TransferredOut" style="display: none;position: fixed ; width: 70%;height: 70vh;top: 50%;
                                    left: 50%;transform: translate(-50%,-50%);background-color: white;border: 1px solid black">
            <button id="closeOtBtn" >x</button>
            <h1>Transferred Out List</h1>
            <form action="ChangeDepartment">
                <table style="text-align: center" class="empTable">
                    <th>Employee Id</th>
                    <th>To Department Id</th>

                    <c:forEach items="${transferredInListVar}" var="var">
                        <%
                            for (Transferred bean : (List<Transferred>) request.getAttribute("transferredInList")) {
                                out.println("<tr >");
                                out.println("<td>" + bean.getEmpId() + "</td>");
                                out.println("<td>" + bean.getToDepId() + "</td>");
                                out.println("</tr>");
                            }
                        %> 
                    </c:forEach>
                    ...
                    <input type='hidden' name='empCatId' <% out.print("value='"+emp.getEmpJobCategory().getCatId()+"'"); %>/>
                    <input type='hidden' name='sendFrom' value="director_two_answer"/>
                    <input type="number" name="empId" required="true" />
    ...
    <script>
        ...
        var tranOut = document.getElementById('TransferredOut');
        var butnOt = document.getElementById('showOtBtn').addEventListener("click",showOtDialog)
        var closeButnOt = document.getElementById('closeOtBtn').addEventListener("click",closeOtDialog)
        ...
        function showOtDialog(){
            tranOut.style.display = 'block';
        }
        function closeOtDialog(){
            tranOut.style.display = 'none';
        }
